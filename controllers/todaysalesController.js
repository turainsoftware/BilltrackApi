...
const today_invoice__list = async (req, res) => {
    try {
        const user = req.user;
        ...
        const query = "SELECT company_name_id, name FROM user WHERE user_id = ?";
        ...
        const [invoice_list] = await db.query(invoice_list_query, [user.id]);
        ...
        // Send response including generated_by only if invoices exist
        res.status(200).json({
...
const today_invoice_amount_details = async (req, res) => {
    try {
        const user = req.user;
        ...
        const invoice_list_query = `
            ...
            AND cancel_status = 0 AND DATE(invoice_date) = CURDATE()`;
...
module.exports = { today_invoice__list,today_invoice_amount_details};